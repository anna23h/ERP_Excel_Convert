...
界面布局：**顶部「① 输入文件」共用区** + 下方**四个标签页**（阶段一 / 阶段二 / 货代合并 / 箱单），**右侧常驻运行日志**。操作顺序：
...
6. **箱单标签页**（B2B 出口，与拉单无关，独立使用）：选 ERP 导出的 `sale.order` 行明细 → 出箱单半成品，品名/SKU/条码/HS/原产国已填好，托盘/批次/箱号/箱规/箱数等留空给仓库手填。要几张 SO 合并成一张箱单，就在 ERP 里一次性导到同一份文件里。
7. 点右下「📂 打开输出文件夹」取结果。运行过程看右侧日志。
