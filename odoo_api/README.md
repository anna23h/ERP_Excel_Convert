...
**2026-08-23 用户拍板**：预测库存会掺进虚拟库存，派生指标（`可撑周数` / `可撑峰值周数` /
`缺口` / `峰值缺口`）一律以**在手库存**为分子。机制上也站得住——天猫的处罚触发于
「此刻能不能发货」，在供应商手里或路上的货今天发不出去。

`在途入库(仅参考)` / `待出库(仅参考)` 两列保留为**事实**，但**不参与任何派生指标**。
`预测库存` / `预测缺口` 两列已移除。

⚠ **换成在手是个双向的取舍，方向要知道**。预测库存 = 在手 + 在途 − 待出：
在途会**虚高**（这是移除的理由），但待出会**压低**，而待出是**真实的已下单未发货**。
样本 170 个里 **94 个的在手 > 旧预测**（有订单压着），只有 10 个是在途拉高的。
后果：**接不住峰值周的 SKU 从 111 个减到 86 个**——用在手看起来比用预测更安全。

被"放过"的 25 个里有真风险，例如 `Femibion_15199958`：在手 128，但**已预留 108**，
真正能接新订单的只有 20，而峰值周要 124 件。

所以给了 `--stock-basis`：

| 取值 | 分子 | 接不住峰值的 SKU（样本 170） |
|---|---|---|
| `onhand`（默认） | 在手库存 | 86 |
| `available` | 在手 − 已预留 | **111** |

**两者都不含在途，都没有虚拟库存问题**；差别只在要不要认「已被订单吃掉」。
（实测 `已预留` 与 `待出库` 在 170 个样本里有 143 个完全相等，是同一件事的两种记法。）

## 归因口径：宽 or 窄

**2026-08-23 最终口径：`narrow`（默认）——「晚于应发货时间」不算缺货类。**
曾一度改为 `wide`（用户提出实际操作中它大部分可归因为缺货），实测后**否决**。

| 口径（2026-06-01 起，可归 SKU 的 2087 条 / 1,738 EUR） | 单数 | 占比 | 罚款 | 占比 | SKU |
|---|---|---|---|---|---|
| `narrow`（只算标签明确写缺货的四类） | 313 | 15.0% | 937 EUR | 53.9% | 183 |
| `wide`（默认，+晚于应发货/最晚发货） | 1994 | **95.5%** | 1,695 EUR | **97.5%** | **540** |

**否决理由**：归因面扩到 95.5% 的单数后，「是不是库存问题」这个筛子几乎不再筛任何东西
——分类退化成"我们被罚了很多"的同义反复。**分类的价值在于能排除。**
（放宽确有收益：多捞出 357 个窄口径看不见的 SKU、合计 403 EUR，排名顶部稳定
前 10 重叠 8/10，`DMS_801810x2` 会从第 32 名跳到第 3。要对比可以 `--stock-violation-mode wide`。）
周报仍保留 `明确缺货罚款EUR` 列。

`晚于应到仓时间` 不归入任何一档——那是发货之后的运输段，不是库存问题。

## 补货优先级：近期还在不在卖

**近两周没销量的不该优先补货**——但这条规则有个会反噬的陷阱，实现时必须绕开。

⚠ **零销可能正是缺货造成的。** 2026-08-23 实测：近两周零销的 511 个 SKU 里 **201 个在手为 0**。
天真地按「近两周无销量 → 降级」处理，等于**让缺货自我强化**：
卖光 → 没销量 → 判定不重要 → 不补货 → 继续缺货。
典型 `Pollival_13748591`：窗口内卖过 254 件、峰值周 39、有缺货处罚记录，
现在在手 0、近两周零销——天真规则会把它降级，而它零销的原因正是我们自己断了货。

`补货优先级` 五档（`--recent-weeks` 默认 2、`--new-days` 默认 60）：

| 档位 | 判据 | 含义 |
|---|---|---|
| `1-在卖` | 近 N 周有销量 | 优先 |
| `2-新品` | 首次产生销量 < N 天 | 还没跑起来，**不因零销降级** |
| `3-待观察` | 近 N 周零销 **且无货** | **不降级**——零销可能是缺货压住的，交人判断 |
| `4-降级` | 近 N 周零销 **且有货** | 有货卖不动，需求确实弱 |
| `5-无销量` | 窗口内完全没卖过 | 最低 |

实测（6 月至今窗口内有销量的 SKU）：`1-在卖` 469、`2-新品` 26、`3-待观察` 185、`4-降级` 289。

**新品识别不能用 `product.product.create_date`**：实测有销量的 969 个 SKU **全部**建档 >90 天
（ERP 记录远早于实际上架，那个字段识别不出新品）。改用 `sale.report` 的 `date:min`
（首次产生销量的日期），实测分布合理：≤30 天 16 个、30–60 天 21 个、60–90 天 15 个。
...
| `--stock-basis` | `onhand` | 派生指标分子：`onhand` / `available`(在手−已预留) |
| `--stock-violation-mode` | `narrow` | 缺货归因口径（`wide`/`narrow`，见上节） |
| `--recent-weeks` | 2 | 判断「近期是否还在卖」的窗口周数 |
| `--new-days` | 60 | 首销在这么多天内视为新品，不因零销降级 |
