...
天猫后台无法按店筛，完整天猫导出/面单已完成名单都含**两店**——经连接键 `∩ ERP`（单店）天然收敛到该店，
...
- **无运单**：既不在名单也未取消。→ **从拣货/面单剔除**，回传ERP上传表(无运单类，Terms 前缀 `无运单`，已带则不重复加)。
...
- **无货勾选**(黑名单)：仓库返回标 0/1 的文件，**直接取有货(0)** 作发货集合（`classify_return`）。
  ...
  - 不再用「候选 − 无货」减法：漏返回某店/某单不会默认全发，更安全。`--done` 仅作交叉核对(发了候选外的单则报警)，非必需。
