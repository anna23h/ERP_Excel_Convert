...
`procure/gap_report.py`。给定若干销售单 → 一张 5 sheet 的 xlsx：汇总对账（真相源）、
每张单一张备货进度、报价比较（产品 × 供应商）、缺口明细。
走 `odoo_api` 只读拉数。跑法与产出清单见
[运行指令 · 十一](启动说明/运行指令.md)。

**「询价-<供应商>」一家一张的底稿已于 2026-08-30 删除**（用户裁定：按供应商看是在
报价比较上筛一次的事，多 6 张 sheet 只增加理解成本）。报价比较随之从只读汇总升为
**唯一录入口**，每家 9 列 = 4 列 ERP 采购历史（次数/最近日期/最近单价/最近数量，重跑刷新）
+ 5 列人填（报价/可供/保质期/交期/备注，重跑保留）。旧版产出里填在底稿上的内容仍会被读回，
改版不丢。
...
- [ ] **采购缺口 + 报价比较**（`procure/gap_report.py`）：销售单 → 缺口（需求 − max(0, 在手 − 他单占用)，四个中间量各占一列可复核）× 采购历史（3 个月内每家的次数/最近日期/单价/数量，窗口为空回落历史最近一次并标灰）→ 汇总对账 + 每单一张备货进度 + 报价比较（产品 × 供应商，每家 9 列 = 4 列采购历史 + 5 列人填，**唯一录入口**）+ 缺口明细，共 5 张 sheet（一家一张的「询价-x」底稿 2026-08-30 删除）。走 `odoo_api` 只读。重跑按 `产品代码 (+供应商)` 回收上一版六类手填内容（报价/可供/保质期/交期、分给本单、已下单量），`--fresh` 关掉。**已下单量刻意不从 Odoo PO 取**——本库 PO 是到货后才建的（2026-08-27 实测近 90 天 707 张 PO，95% 已 done、86% 的行全额收货），当下单进度用滞后一个采购周期。**S04018+S04029 实测跑通（2026-08-30 复跑：23 产品 / 17 有缺口 / 报价比较覆盖 6 家供应商 / 59 列），等实际询价一轮后再校准。**
