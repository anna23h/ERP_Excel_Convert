...
- **录入 = Jürgen 的 `260825 Order template.xlsx`**，字段名与顺序原样（`Product` / `PZN` /
  ...
  **自动的**品名（中）/ 已配 / 缺口 / 已到（灰底标明「别手填」）。
  `Product`（德语名）也由 PZN 自动查出，但**这格可以手打覆盖**——非药房品没有 PZN，
  覆盖掉公式正是预期用法，只影响那一列。
- **⚠ 判「这一列在不在用」必须比值，不能用 `COUNTA`**：`COUNTA` 把「有公式但结果是空
  字符串」的格子也算作非空，而 Product / 品名（中）行本身就是公式 —— 守卫会因此恒为真，
  空列的已配与已到一律显示成 `SUM(空格子)=0`。用 `AND(B2="",B3="")`。
  （2026-09-23 用户实测报出：他给 Product 行补了德语名公式，11、13 两行立刻冒 0，
  而 12 行不冒——因为缺口那道守卫多一层 `B7=""`。见 `ISSUES.md` 的 L 条。）
...
- **总览是只读的**：每行自动取自录入的一列。空行是预留产品位，用筛选去掉。
  **排序已被工作表保护挡住**——直接排会把公式排乱，要排序先复制 → 选择性粘贴为「值」。
- **锁与输入约束**（2026-09-23 加）：
  · 锁死的只有四行——品名（中）/ 已配 / 缺口 / 已到，外加整张 `总览` 与 `产品字典`。
    `Product` 行**故意不锁**：它虽是公式，但设计上允许手打覆盖。`供应商` 表不锁（要加名单）。
  · **保护无密码**，目的是防手滑不是防人。要插行插列加字段，「撤消工作表保护」点一下，
    改完再保护（`审阅` → `保护工作表`，同样不设密码）。
  · PZN 只能填 1~99999999 的整数；需求量 / 各家 `Quantity` / `received quant.` 只能填非负整数；
    `AEP / HAP` 与各家 `price` 只能填非负数字。**`ETA` 与两个日期行不设验证**——
    真实数据里 `requested` / `??` 比日期还多，卡死只会逼人去关验证。
  · **一个格子只能挂一条数据验证**，所以「PZN 不在字典里」从验证改成了**条件格式变橙**：
    垃圾输入直接挡，陌生新品只提示不拦（字典是药房产品快照，新品不该被挡住）。
  · ⚠ 以上只在**桌面版 Excel** 实测过；Excel Online 是否同样拦得住**未验证**。
- **⚠ openpyxl 写条件格式的填充要用 `PatternFill(bgColor=…)`**：条件格式走差异格式（dxf），
  写成平时的 `PatternFill("solid", fgColor=…)` 规则会照样命中、但画出来是「无填充」，
  看上去就是规则没生效。因此验证脚本查的是 `DisplayFormat.Interior.Color`（真实显示色），
  **不是**「规则在不在」——后者查不出这个 bug。
...
| 产出 | 内容 | 下一步手工动作 |
|---|---|---|
| `新订单获单清单[店].xlsx` | 系统履约单号（履约单状态=新订单 ∩ ERP）| 复制 → 天猫后台批量获单。**两店同批合并一份**（文件名不带店），单店批次带店名 |
...
| `回传ERP销售上传表[店].xlsx` | 取消/无运单/已补运单三类 Terms 写回**一张** | 上传 ERP，按关键词分别 Cancel/标记/恢复。**两店同批合并一份**（文件名不带店），单店批次带店名 |
...
`procure/gap_report.py`。给定若干销售单 → 一张 6 sheet 的 xlsx：汇总对账（真相源）、
每张单一张备货进度、询价录入（长表，唯一录入口）、报价比较（只读派生）、缺口明细。
走 `odoo_api` 只读拉数。跑法与产出清单见
[运行指令 · 十一](启动说明/运行指令.md)。

**录入与比价按各自的形状拆开**（2026-08-30 定案）：`询价录入` 是**长表**——一行一个
「产品 × 供应商」，唯一录入口，按供应商看走 Excel 自带筛选；`报价比较` 瘦成 3 + N 列的
**只读派生视图**，值用 `SUMIFS/COUNTIFS` 从长表实时取，**填完报价当场就变、不必重跑**。
为什么不继续用宽矩阵：矩阵宽度由**供应商家数**决定，信息量却由**实际发生过的组合数**决定，
实测两者差 3 倍（59 列 vs 33 个组合），且 17 个产品里 8 个只有唯一一家、根本无从「横着比」。
长表还顺手补上一个窟窿——旧版供应商列是从有采购历史的家推出来的，
**想问一家没买过的新供应商压根没地方填**；现在自己加一行即可，重跑保留、标『新』。
（一家一张的「询价-x」底稿已于同日删除；旧版产出里的手填内容仍会被读回，改版不丢。）

表上方**没有标题/口径/时间戳行**，第 1 行即表头（2026-08-30 用户裁定去掉；生成日期在文件名里，
口径打在终端上）。

`汇总对账` 上**每张 SO 一列售价**（`sale.order.line.price_unit`，**未税、未扣行折扣**）
+ `毛利额` / `毛利率`（= 最低售价 − 最低报价，几张单卖价不一时按最低算，口径保守）。
没有售价就没法判断「这家报的价能不能接」——实测两单都有的产品 4 个里就有 1 个两单不同价，
所以按单分列而不是合成一列。**若将来出现行折扣或多币种，这一列会静默失真**
（实测当前 discount 全 0、均为 EUR）。

`已报价家数` / `最低报价` **从长表实时取**（`COUNTIFS` / `MINIFS`），
询价进度直接摆在真相源上，不必翻长表。**`选定数量` 与 `已下单量` 刻意分成两列**：
前者是「打算给这家多少」、后者是「实际下出去多少」，`剩余` 只认后者——防重复下单这条线
必须踩在实际发生的量上；中间那段看 `未下单`。`MINIFS` 需 Excel 2019/365（Excel Online 有）。

三条容易踩的口径：**报价比较是候选展开不是分配**（同一缺口同时挂在几家名下，都写总缺口，
靠汇总对账的「已下单量/剩余」防重复下单）；**「总可用」是几张单共用的池子**，分配手填；
**已下单量不从 Odoo 采购单取**——本库 PO 是到货后才建的，当下单进度用会滞后一个采购周期。

重跑时自动回收上一版的手填内容（报价/可供/保质期/交期、分给本单、已下单量），
所以「随订货阶段推进」就是重复跑同一条命令。**同时出一份英文只读快照**
`Procurement-Gap-…xlsx` 供分发给同事（手填只认中文版，`--no-en` 可关）。

**没有 GUI 入口、不打包给同事**（2026-08-27 用户定）：它要 Odoo API 密钥，进同事的 exe
等于把凭据链路铺到办公室机器。要在公司 Windows 上跑，见
[运行指令 · 十一 · 在 Windows 上跑](启动说明/运行指令.md)。

## 采购价格收集表（成本核算用）

`procure/po_price.py`。给定若干销售单 → 一张单 sheet 的 xlsx，**一个产品一行**，把它近 N 个月
（默认 3）在 ERP 里的**每一笔**采购摊在这一行上，供人工核算成本。跑法见
[运行指令 · 十二](启动说明/运行指令.md)。

**这条流水线只收集整理，不做任何计算判断**（2026-09-02 用户用改过的样表定的口径）。
表上「成本」列**脚本一个字都不写**，留空由人填。为什么不能算：同一产品在窗口内往往有好几个价，
销售数量常常要**跨价分摊**——实测 Gehwol 那行人是这么填的：`70*4.63` 换行 `74*4.68`
（144 件按两个实际进价拆开）。这既不是一个数、也不是任何一种平均，脚本猜哪种都是错的。
**同理不出加权均价**：均价是算出来的结论，不是收集到的事实，摆在证据表上只会诱导人照抄
（初版有这一列，被用户删掉）。
...
| 段 | 列 | 来源 |
|---|---|---|
| 销售侧 | 销售单 / 产品代码 / 产品名称 / `销售数量` | `sale.order.line`，同产品跨分节**合计** |
| **人工** | `成本` | **脚本留空**，自由文本，可多行写分摊式 |
| 采购汇总 | 采购笔数 / 采购总量 / 最低价 / 最高价 / 最近采购日期 / 最近采购价 | 由逐笔聚合，**只有极值与计数** |
| 逐笔 | 采购日期 / 供应商 / 采购单号 / 采购数量 / 采购单价 | `purchase.order.line`，多行单元格 |

**逐笔五列是多行单元格，同一格内一笔一行、五列行行对齐**，横着读就是一笔完整记录。
列宽都调到不换行——**一旦某格换行，这五列的对齐当场就散**，这是本表唯一的排版要害。
所以采购单号只留 P 编号：`order_id` 的显示名后面还挂着供应商发票号
（`P11836 (Proforma Rechnung Nr. 2007825273 (P11836) Rechnung Nr. 45107763)`，70+ 字符）。

**销售数量按产品合计，拆分写进备注**：本库 SO 用 `line_section` 按**客户的每张采购订单**分节，
同一产品在几张客户订单里各占一行（实测 S04042 的 Ferrum 是 5000 + 3200）。不写拆分的话，
那个 8200 从哪来的没法复核。

**每次都是空白新表**（2026-09-02 用户定）：不像 `gap_report` 那样回收上一版手填内容——
一张销售单只核算一次，回收机制在这里是多余的复杂度。旧产出不会被覆盖（同名自动加序号），
填过的表放在那儿就是账。

**与 `gap_report` 的分工，两者不合并**：那张管「还缺多少、找谁买、什么价位」（决策前），
这张管「已经买过哪些、什么价」（核算时）。受众和时点都不同。

口径与 `gap_report` 共用的两条：只认 `state ∈ {purchase, done}` 的采购行（草稿/询价没成交，
价格不作数）；剔除 `common/vendor.NOISE_VENDOR_PATS`（Alibaba 名下是我方客户的退货包裹单、
VO Test Order 是虚拟库存映射测试单，单价恒为 0，不剔会把最低价一路拉到 0）。
这份名单**全仓只此一份**——2026-09-22 之前是四处各写各的，结果同一客户的新加坡法人实体
（`Alibaba.com Singapore …`）漏了三处，实测把 39 个 SKU 的最低价拉成 0，
故只按 `Alibaba` 子串匹配、不写全名。
**一张采购单里同一产品拆成的几行逐行保留**（实测 P11255 的 20 件 + 120 件）——
这是收集表，合并就等于替人做了判断。
...
- [x] **新订单获单清单**（缺口补全）：`履约单状态=新订单` ∩ ERP → 系统履约单号，桥接「确认 order」与「天猫批量获单」两个手工步。两店同批合并一份（2026-08-29）。
- [x] **回传ERP销售上传表**（三合一）：取消/无运单/已补运单三类 Terms 写回合并**一张**（替代原 取消单/无运单清单 两文件）。两店同批也合并成一份（2026-08-29）。
...
- [ ] **采购缺口 + 报价比较**（`procure/gap_report.py`）：销售单 → 缺口（需求 − max(0, 在手 − 他单占用)，四个中间量各占一列可复核）× 采购历史（3 个月内每家的次数/最近日期/单价/数量，窗口为空回落历史最近一次并标灰）→ 汇总对账 + 每单一张备货进度 + 询价录入（长表，一行一个「产品×供应商」，**唯一录入口**，可自己加行问新供应商）+ 报价比较（3+N 列**只读派生**，SUMIFS 从长表实时取）+ 缺口明细，共 6 张 sheet；汇总对账另有 已报价家数/最低报价（从长表实时取）与 未下单/剩余 两条口径不同的差额列（一家一张的「询价-x」底稿 2026-08-30 删除）。走 `odoo_api` 只读。重跑按 `产品代码 (+供应商)` 回收上一版六类手填内容（报价/可供/保质期/交期、分给本单、已下单量），`--fresh` 关掉。**已下单量刻意不从 Odoo PO 取**——本库 PO 是到货后才建的（2026-08-27 实测近 90 天 707 张 PO，95% 已 done、86% 的行全额收货），当下单进度用滞后一个采购周期。**S04018+S04029 实测跑通（2026-08-30 复跑：23 产品 / 17 有缺口 / 询价录入 33 行 / 6 家供应商），等实际询价一轮后再校准。**
