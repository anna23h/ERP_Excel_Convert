...
| `procure/` | **采购缺口 + 询价底稿**（销售单 → 每产品缺多少 × 曾在哪几家买过 → 按供应商分发的询价底稿；询价/分配/已下单量手填，重跑不冲掉） | `python3 -m procure.gap_report S04018 S04029`（无 GUI，需 Odoo 凭据） | [下方章节](#采购缺口--询价底稿销售单驱动) + 脚本 docstring | 建设中 |
...
`procure/gap_report.py`。给定若干销售单 → 一张多 sheet 的 xlsx：汇总对账（真相源）、
每张单一张备货进度、报价比较（产品 × 供应商）、缺口明细、每供应商一张询价底稿。
走 `odoo_api` 只读拉数。跑法与产出清单见
[运行指令 · 十一](启动说明/运行指令.md)。

三条容易踩的口径：**底稿是候选展开不是分配**（同一缺口出现在多家底稿上，都写总缺口，
...
- [ ] **采购缺口 + 询价底稿**（`procure/gap_report.py`）：销售单 → 缺口（需求 − max(0, 在手 − 他单占用)，四个中间量各占一列可复核）× 采购历史（3 个月内每家的次数/最近日期/单价/数量，窗口为空回落历史最近一次并标灰）→ 汇总对账 + 每单一张备货进度 + 报价比较（产品 × 供应商）+ 缺口明细 + 每供应商一张询价底稿。走 `odoo_api` 只读。重跑按 `产品代码 (+供应商)` 回收上一版六类手填内容（报价/可供/保质期/交期、分给本单、已下单量），`--fresh` 关掉。**已下单量刻意不从 Odoo PO 取**——本库 PO 是到货后才建的（2026-08-27 实测近 90 天 707 张 PO，95% 已 done、86% 的行全额收货），当下单进度用滞后一个采购周期。**S04018+S04029 实测跑通（23 产品 / 16 有缺口 / 5 张底稿），等实际询价一轮后再校准。**
